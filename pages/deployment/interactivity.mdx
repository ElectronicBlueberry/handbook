...
<VideoComponent src="/video/client-side-variables.mp4" width="1250" height="616" />
...
// Embeddable React component defined as per example below
<Embeddable token={securityToken} onVariablesChange={handleVariableChange} />;
```
...
// Embeddable React component defined as per example below
<Embeddable token={securityToken} onEmbeddableError={handleEmbeddableError} />;
```
...
<>
  ...
  <Embeddable token={securityToken} onComponentsLoad={handleComponentsLoad} />
</>;
```
...
const Embeddable: React.FC<Props> = ({
  ...
  function handleVariableChange(e: { detail: Detail[] }) {
    if (onVariablesChange) {
      onVariablesChange(Object.fromEntries(e.detail.map((c: any) => [c.variableName, c.newValue])));
    }
  }
