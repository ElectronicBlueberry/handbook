...
<Callout emoji="💡">
  If you start in the **Data Model Editor**, you can always export them as YAML to your code
  repository later.
</Callout>
...
<Callout emoji="💡">
  A common misconception is that you have to write **GROUP BY** statements in your `sql`. Instead,
  use `sql` to define a base table and then separately define [dimensions and
  measures](/data-modeling/dimensions-and-measures). The data modeling layer automatically generates
  the necessary SQL, including GROUP BY clauses, behind the scenes.
</Callout>
