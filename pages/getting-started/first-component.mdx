...
<Callout emoji="💡">
  The `.emb.ts` file is a special **companion file** that Embeddable looks for. Embeddable will only
  pay attention to files that are reachable from `.emb.ts` files.
</Callout>
...
<Callout emoji="💡">
  Learn more about [defining components](/development/defining-components), [loading
  data](/development/loading-data) and [adding interactivity](/development/interactivity).
</Callout>
