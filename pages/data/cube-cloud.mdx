...
1. **REST API Endpoint**
   ...
   <Callout emoji="💡">Copy only the URL segment before cubejs-api/v1.</Callout>
...
Firstly, we'll create a new branch called `add_completed_percentage_measure`. In this branch, we'll add a new **_"completed percentage"_** measure to our `base_orders` model and include it in the `orders` [**view**](/data-modeling/views).

<VideoComponent src="/video/testing-multiple-data-providers.mp4" width="1920" height="1080" />
...
<VideoComponent src="/video/staging-url-and-secret.mp4" width="1920" height="1080" />
...
<VideoComponent src="/video/testing-data-providers-in-sc.mp4" width="1920" height="1080" />
...
const validateEmbeddableJwt = (auth, req) => {
  try {
    ...
    throw new AuthHandlerError(
      403,
      `validateEmbeddableJwt JWT validation failed: ${err.message}, ${auth}`,
    );
  }
};
...
This is an example `cube.js` file. Copying and pasting this file probably _will_ work for you, although you may want to modify it!
