...
<Callout emoji="💡">
  this document is transcribed from the video and therefore at times makes reference to things you
...
```tsx
ChartJS.register(CategoryScale, LinearScale, PointElement, Filler, Legend, TimeScale);
```
...
useEffect(() => {
  ...
  if (props.results?.data?.length > 0) {
    const labels = props.results.data.map((item: any) => item[props.slice.name]);
    const values = props.results.data.map((item: any) => item[props.metric.name]);
