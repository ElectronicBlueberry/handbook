...
<Callout emoji="💡">
  Client Context **isn’t** available in your data models or server-side security checks. It’s
  front-facing data only - avoid passing sensitive information. For that, see [Security
  context](/data-modeling/row-level-security#security-tokens-and-security-context).
</Callout>
