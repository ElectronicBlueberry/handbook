...
- `select` is an array of [dimensions](/data-modeling/dimensions-and-measures#dimensions), [measures](/data-modeling/dimensions-and-measures#dimensions) and _time dimensions_ (time-based dimensions, often paired with a **granularity**).
...
- **`notInDateRange`**: Property is outside the specified date range.
  ...
  <Callout emoji="💡">
    You can also specify a wide range of relative time strings e.g. yesterday, last week etc. Behind
    the scenes, Cube uses [Chrono](https://github.com/wanasit/chrono) to parse these.
  </Callout>
...
export default function DynamicLoader(props) {
  ...
  return (
    <div>
      <Table data={data} />
      <button onClick={() => applyFilter('United States')}>Filter to US</button>
    </div>
  );
}
```
...
<Callout emoji="💡">
  Importantly, these code-driven techniques are useful for **component-specific** actions, such as
  pagination or custom filters controlled entirely inside the component. For broader
  **dashboard-level** filters, the no-code builder already provides a visual interface—using
  **variables** and **filters** to reload component data without writing extra code. This gives you
  the best of both worlds: a powerful no-code approach for general filtering, and full control for
