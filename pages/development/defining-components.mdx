...
### **Push to Embeddable** Run `npm run embeddable:dev` to test out your component, and when you’re
happy with it, [**push**](/development/pushing-code) your component code to Embeddable. Your
teammates can now **drag, drop, and configure** it in their dashboards - no extra coding required.
...
export default defineComponent(Component, meta, {
  props: (inputs: Inputs<typeof meta>) => {
    return {
      ...
      results: loadData({
        // fetches data from your database and passes it to your component
        from: inputs.ds,
...
<Callout emoji="⚠️">
  <p>
    <strong>Important:</strong> if you follow this route, you will no longer receive updates to the
    original component. They'll still come through when you update the package, but because this
    involves disabling a particular chart, you won't see them.
  </p>
</Callout>
