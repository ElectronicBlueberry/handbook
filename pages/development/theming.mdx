...
<VideoComponent src="/video/theme-example.mp4" width="800" height="450" />

<Callout emoji="⚠️">
  Were you working with the previous version of Vanilla components that required you to clone that
  entire repo? [Go here for full instructions on switching!](/development/theme-welcome.mdx)
</Callout>
...
<Callout emoji="💡">
  You can find the full list of available `Theme` properties (and their default values)
  [here](https://github.com/embeddable-hq/vanilla-components-v1/blob/main/src/themes/defaulttheme.ts).
</Callout>
...
<Callout emoji="🚧">
  Currently, your font choice impacts the entire dashboard. In the future, we will support the
  ability to set different fonts for different parts of the dashboard, such as titles, descriptions,
  etc.
</Callout>
...
The good news is: you don’t have to create or update all of these yourself. In fact, you don’t have to update _any_ of them yourself. They’re auto-generated. All of these variables are prefaced with `embeddable` to help avoid conflicts, and they are structured exactly like the theme object. Here is a very small example of them:
...
export default (props: Props) => {
  ...
  return <Pie height="100%" options={options} data={data} onClick={() => null} />;
};
```
...
Please note: `useTheme` runs before render, unlike most React hooks, so you will _always_ have access to a theme if you have one defined (if you don’t use vanilla components at all, you’ll need to define one. We’ll have separate documentation on that soon).
