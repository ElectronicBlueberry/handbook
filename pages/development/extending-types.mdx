...
<Callout emoji="⚠️">
  When extending native types, best practice is to create a new `type.emb.ts` file as if you were
  creating a custom type, but avoid using the `defineType` function, as this will cause build
  errors. The native type is already defined in the core library. You only need the `defineOption`
